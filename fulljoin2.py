...
import generate_spatial_proximity_degree as gspd
...
# 调用数据
E = gd2.data_exa_2  # 实例poi坐标 用于邻居矩阵
EM = gd2.data_mem_2  # 实例poi犹豫模糊隶属度 用于记分函数
EP = gd2.data_poi_2  # 实例poi 用于空间邻近度
ET = gd2.data_fea_2  # 特征 用于行表实例
...
distance = {'distance_d': distance_dd}
...
# 生成犹豫模糊空间邻近度
proximity_matrix_pm_p = gspd.generate_spatial_proximity_degree(EP, distance, membership)

instanceInfo = np.zeros((1, N))
for i in range(0, N):
    instanceInfo[0, i] = i + 1

proximity_matrix_pm = {'instanceInfo': instanceInfo, 'proximity_mp': proximity_matrix_pm_p}
...
for i in range(0, K):  # 循环获取实例序号 外层循环特征
    ins = list()

    for w in range(0, N):  # 内层循环实例
        T1_co_location.loc[i, 'label'] = C1_pattern.iloc[i, 0]

        if C1_pattern.iloc[i, 0] == E.iloc[w, 1]:

            if not math.isnan(E.iloc[w, 0]):  # 删除poi空序号
                T1_e = list()
...
while not P[k]['pattern'].empty and k < 2:
    ...
    # 生成表实例方法
    temp_gct = gct.generate_co_location_table_instance(min_prev, C, T_co_location, k, proximity_matrix_pm, f_i_pr,
                                                       membership)
